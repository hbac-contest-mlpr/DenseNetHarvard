...
def main():
    ...
    if USE_SUBSET:
        print(
            f"Using subset of {LEN_SUBSET} samples only! Picked the first {LEN_SUBSET} samples."
        )
    ...
    model = epicmodel.DenseNet(
        layer_num=(16,), growth_rate=32, in_channels=4, classes=6
    )  # model
    ...
    try:
        for epoch in range(MAX_EPOCHS):
            ...
            print(f"Epoch: {epoch}")
            epoch_statistics["epoch"] = epoch
            epoch_statistics["test_size"] = len(test_batches)
            epoch_statistics["train_size"] = len(train_batches)
            ...
            for batch_idx, (x_train, y_train) in enumerate(train_batches):
                ...
                train_classes.extend(
                    list(np.argmax(y_pred.cpu().detach().numpy(), axis=1))
                )
                pred_classes.extend(
                    list(np.argmax(y_train.cpu().detach().numpy(), axis=1))
                )
                ...
                if batch_idx % PRINT_EVERY_BATCH == 0:
                    print(
                        f"\t Batch_idx: {batch_idx} | Batch Loss: {loss:.5f} (time: {time.time()-bt:.2f}s | {time.time()-t0:.2f}s elapsed)"
                    )
                bt = time.time()
            overall_loss /= len(train_batches)

            epoch_statistics["train_loss"] = (
                overall_loss.cpu().detach().numpy().tolist()
            )
            ...
            with torch.inference_mode():
                ...
                for batch_idx, (x_test, y_test) in enumerate(test_batches):
                    ...
                    test_classes.extend(
                        list(np.argmax(test_pred.cpu().detach().numpy(), axis=1))
                    )
                    pred_classes.extend(
                        list(np.argmax(y_test.cpu().detach().numpy(), axis=1))
                    )
        ...
        print(f"TRAINING INTERRUPTED, SAVING MODEL")
        epoch_statistics["saved"] = True
        torch.save(
            {
                "model_state_dict": model.state_dict(),
            ...
            f"./saved_models/{MODEL_PREFIX}{epoch}_INTERRUPT.pth",
        )
        with open(f"{STATS_SAVE_PATH}stats_{MODEL_PREFIX}_INTERRUPT.json", "w") as f:
            json.dump(epoch_statistics, f)
