...
class DenseLayer(nn.Module):
    def __init__(
        self, in_channels, middle_channels=128, out_channels=32
    ):  # should we put middle_channels here?
...
class DenseBlock(nn.Sequential):
    def __init__(self, layer_num, growth_rate, in_channels, middle_channels=128):
        ...
        for i in range(layer_num):
            layer = DenseLayer(
                in_channels + i * growth_rate, middle_channels, growth_rate
            )
...
class DenseNet(nn.Module):
    def __init__(
        ...
        self.DenseBlock1 = DenseBlock(
            layer_num[0], growth_rate, self.feature_channel_num, middle_channels
        )
        ...
        self.DenseBlock2 = DenseBlock(
            layer_num[1], growth_rate, self.feature_channel_num // 2, middle_channels
        )
        self.feature_channel_num = (
            self.feature_channel_num // 2 + layer_num[1] * growth_rate
        )
        self.Transition2 = Transition(self.feature_channel_num)

        self.DenseBlock3 = DenseBlock(
            layer_num[2], growth_rate, self.feature_channel_num // 2, middle_channels
        )
        self.feature_channel_num = (
            self.feature_channel_num // 2 + layer_num[2] * growth_rate
        )
        self.Transition3 = Transition(self.feature_channel_num)

        self.DenseBlock4 = DenseBlock(
            layer_num[3], growth_rate, self.feature_channel_num // 2, middle_channels
        )
        self.feature_channel_num = (
            self.feature_channel_num // 2 + layer_num[3] * growth_rate
        )
